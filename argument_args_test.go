...
func (b TestBroker) MarshalText() ([]byte, error) {
...
// TestBrokers is a slice type that implements TextUnmarshaler on the slice itself
// This mimics the behavior of kafka.Brokers where the entire slice is parsed from
// a comma-separated string
type TestBrokers []TestBroker

func (b *TestBrokers) UnmarshalText(text []byte) error {
	value := string(text)
	if value == "" {
		*b = TestBrokers{}
		return nil
	...
	*b = make(TestBrokers, len(parts))
	for i, part := range parts {
		trimmed := strings.TrimSpace(part)
		if err := (*b)[i].UnmarshalText([]byte(trimmed)); err != nil {
			return err
		}
	}
	return nil
...
func (b TestBrokers) MarshalText() ([]byte, error) {
	strs := make([]string, len(b))
	for i, broker := range b {
		strs[i] = broker.String()
	}
...
var _ = Describe("ParseArgs", func() {
	...
	Context("TextUnmarshaler types", func() {
		Context("single values", func() {
			It("parses broker with schema", func() {
				var args struct {
			...
			It("parses broker without schema (adds default)", func() {
				var args struct {
					Broker TestBroker `arg:"broker"`
				}
		...
		Context("slice of TextUnmarshaler elements", func() {
			It("parses broker slice", func() {
				var args struct {
			...
			It("returns error for invalid URL in slice", func() {
				...
				err := argument.ParseArgs(
					ctx,
					&args,
					[]string{"-urls=https://valid.com,invalid-url"},
				)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("URL must start with http://"))
			})
		...
		Context("slice type implementing TextUnmarshaler", func() {
			It("parses TestBrokers (mimics kafka.Brokers)", func() {
				var args struct {
					Brokers TestBrokers `arg:"brokers"`
				}
				err := argument.ParseArgs(
					ctx,
					&args,
			...
			It("parses TestBrokers from default", func() {
				var args struct {
					Brokers TestBrokers `arg:"brokers" default:"localhost:9092,kafka:9093"`
				}
				err := argument.ParseArgs(ctx, &args, []string{})
				Expect(err).NotTo(HaveOccurred())
				Expect(args.Brokers).To(HaveLen(2))
			...
			It("handles empty TestBrokers", func() {
				var args struct {
					Brokers TestBrokers `arg:"brokers"`
				}
				err := argument.ParseArgs(ctx, &args, []string{"-brokers="})
